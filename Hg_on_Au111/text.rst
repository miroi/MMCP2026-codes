...
All displayed mathematical equations are assigned a stable reference label.
Within an RST/Sphinx document, an equation can be referenced with
``:eq:`eq-001``` when the ``sphinx.ext.mathjax`` extension is enabled. The
explicit labels are intended to remain stable even if surrounding prose is
edited.
...
* thermodynamic frequency cutoff: 1 cm\ :sup:`-1`;
...
geometric displacements, eV for energies, cm\ :sup:`-1` for vibrational
frequencies, and K for temperature. Conversion to SI pressure is performed
...
* :math:`E_{\text{HgAu}}` is the potential energy of the optimized Hg/Au slab;
* :math:`E_{\text{Au}}` is the potential energy of the optimized clean Au
  slab; and
* :math:`E_{\text{Hg}}` is the potential energy of an isolated Hg atom
  evaluated with the same MACE calculator.

Negative :math:`\Delta E_{\text{ads}}` therefore denotes energetically
...
and hence :math:`\Delta E_{\text{ads}} = -0.5013228844` eV. Using
96.48533212 kJ mol\ :sup:`-1` per eV gives approximately
:math:`\Delta E_{\text{ads}} = -48.3703` kJ mol\ :sup:`-1`.
...
:math:`q` of dimension :math:`3N_{\text{mobile}}`. The harmonic potential
...
where :math:`H` is the mass-unweighted Cartesian Hessian in the selected
coordinate space.
...
where :math:`d` is the configured displacement in Angstrom. The minus sign
...
formed from the forces at :math:`-2d`, :math:`-d`, :math:`+d`, and
:math:`+2d`. With the sign convention :math:`F = -dV/dq`, the Hessian is
obtained from the negative force derivative. The two-point central formula
remains
...
For a structure with :math:`N_{\text{mobile}}` mobile atoms, the central
two-point Hessian requires :math:`2 \times (3 N_{\text{mobile}})` complete
force evaluations, because each of the :math:`3 N_{\text{mobile}}` Cartesian
...
For Cartesian coordinates :math:`i` and :math:`j` associated with atoms
:math:`a` and :math:`b`,
...
Diagonalization gives eigenvalues :math:`\lambda_k` and normal-mode
...
for positive :math:`\lambda_k`. Frequencies are converted to wavenumbers in
cm\ :sup:`-1`.
...
are retained for thermochemistry. The supplied default is 1.0 cm\ :sup:`-1`.

Significant imaginary modes are not silently converted into thermodynamic
modes. With ``fail_on_imaginary = true`` the calculation stops when a
significant imaginary instability is detected. This is preferable to masking
an unstable optimized structure by simply removing negative frequencies.
...
equal to ``hg_mode_projection_threshold`` with the supplied default value
0.20.
...
energy already included in :math:`\Delta E_{\text{ads}}`; hence the gas term
used here is the thermal ideal-gas contribution rather than a second
electronic atomic-energy term.

6.1 Harmonic oscillator energy spectrum
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

For a normal mode of frequency :math:`\nu`,
...
where :math:`\tilde{\nu}` is the wavenumber in cm\ :sup:`-1`.
...
The implementation does not require :math:`C_V` explicitly for the adsorption
free energy, but this relation is useful for interpreting the temperature
...
For each retained positive frequency :math:`\nu` in cm\ :sup:`-1`, the quantum
of vibrational energy is
...
are stored as wavenumbers in cm\ :sup:`-1`. The photon-like quantum associated
with a vibrational wavenumber :math:`\bar{\nu}` is
...
The same :math:`x` is used in the energy, entropy, and free-energy
expressions.

7.5.2 One harmonic vibrational mode
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

For one mode of quantum energy :math:`\epsilon = h \nu`, the harmonic-
...
``expm1(x)``. For very large :math:`x`, the thermal contribution is
...
Because the zero-point term cancels between :math:`U` and :math:`F`, the same
entropy results whether :math:`U` is written as ZPE plus thermal energy or in
the corresponding partition-function form.
...
:math:`S_{\text{vib}} \to 0`. For a very soft mode, however, the harmonic
...
:math:`\Delta G` calculation, it follows from the same harmonic model and is
useful for interpreting the temperature dependence:
...
:math:`\Delta H - T \Delta S` route and the compact
:math:`\Delta E + \Delta F - \mu` route used in the code.
...
For any vibrational function :math:`X`,
...
For :math:`N` identical monatomic particles in volume :math:`V`, the
...
Using Stirling's approximation for :math:`N!` gives the Sackur--Tetrode form
...
Relative to a reference pressure :math:`P_0`,
...
This term is subtracted from the adsorbed-state enthalpy correction because
the gas reference contains translational PV work whereas the adsorbed atom
does not.
...
For one classical particle in a volume :math:`V`, the translational partition
function is
...
For :math:`N` indistinguishable particles,
...
Using Stirling's approximation for large :math:`N` and the thermodynamic
limit gives the standard ideal-gas expressions used by the Sackur--Tetrode
formulation.
...
:math:`\Delta E_{\text{ads}}`; consequently this gas correction contains only
the thermal ideal-gas contribution.
...
The pressure dependence at fixed :math:`T` is therefore
...
where :math:`P_0` is the configured reference pressure.
...
the adsorption free energy contains :math:`-\mu_{\text{Hg,thermal}}`,
increasing Hg pressure makes adsorption thermodynamically more favorable in
this convention.

9.3.4 Standard pressure versus actual pressure
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The code distinguishes the actual Hg pressure :math:`P` from a reference
pressure :math:`P_0`. The standard-pressure gas entropy is
...
potential :math:`\mu` (Gibbs free energy per particle in the ideal-gas limit).
...
Equivalently, relative to :math:`P_0`,
...
* electronic adsorption energy: :math:`\Delta E_{\text{ads}}`;
* surface/adsorbate vibrational free-energy correction:
  :math:`\Delta F_{\text{vib}}`; and
* gas-phase Hg thermal chemical potential:
  :math:`\mu_{\text{Hg,thermal}}`.

No gas-phase Hg electronic energy is added a second time:
:math:`E_{\text{Hg}}` is already part of :math:`\Delta E_{\text{ads}}`.

10.6 Pressure dependence of Delta G_ads
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

At fixed :math:`T`, the pressure dependence follows directly from the
...
* :math:`\Delta G_{\text{ads}} < 0`: adsorption is thermodynamically
  favorable under the stated :math:`T` and :math:`P` conditions;
* :math:`\Delta G_{\text{ads}} > 0`: desorption is thermodynamically favored
  relative to the chosen gas reference;
* :math:`\Delta G_{\text{ads}} = 0`: adsorption and desorption are at the
...
For each :math:`T` the program evaluates and records at least:

* :math:`\Delta E_{\text{ads}}`;
* :math:`\Delta \text{ZPE}`;
* :math:`\Delta U_{\text{vib,thermal}}`;
* :math:`\Delta U_{\text{vib,total}}`;
* :math:`\Delta F_{\text{vib}}`;
* :math:`S_{\text{gas}}`;
* :math:`\Delta S_{\text{vib}}`;
* :math:`\Delta S_{\text{ads}}`;
* :math:`H_{\text{gas,thermal}}`;
* :math:`\Delta H_{\text{ads}}`; and
* :math:`\Delta G_{\text{ads}}`.

The scan is written to ``hg_au111_thermodynamics_scan.csv`` when the output
prefix is the supplied default. In ``mode = scan`` the values are also printed
in a compact terminal table.
...
sorts the positive temperatures, evaluates :math:`\Delta G` on that grid, and
searches for the first adjacent sign change.
...
:math:`T_{\text{cross}} \approx 235.025029` K.

At the reported solution the residual was approximately
:math:`\Delta G_{\text{ads}}(T_{\text{cross}}) = -3.18 \times 10^{-8}` eV,
with a final numerical bracket of approximately
:math:`[235.02498236,\, 235.02507596]` K.
...
Given a bracket :math:`[T_{\text{low}},\, T_{\text{high}}]` satisfying
...
the new bracket is :math:`[T_{\text{low}},\, T_{\text{mid}}]`; otherwise it
is :math:`[T_{\text{mid}},\, T_{\text{high}}]`. The process is repeated until
either the temperature bracket is narrower than the configured temperature
tolerance or the absolute Gibbs-energy residual is below the configured
Gibbs-energy tolerance.
...
specific model function :math:`\Delta G_{\text{ads}}(T,P)`, not an
independently optimized structure at ``T_cross``.
...
This relation does not by itself give a closed-form :math:`T_{\text{cross}}`
because the vibrational terms are temperature dependent, but it provides a
useful check on pressure-dependent crossover calculations.
...
and ASE's ``IdealGasThermo`` for monatomic Hg(g). The project retains the same
...
* scan temperatures;
* :math:`\Delta G = 0` search activation; and
* MACE model path.

The validation layer ensures, among other things, that the vibrationally
frozen layer count is not smaller than the geometry-relaxation frozen layer
count and does not exceed the number of Au layers.
...
performs the scan, optionally searches for ``T_cross``, and writes the final
JSON results.
...
* minimum frequency: about 10.7 cm\ :sup:`-1`;
* ZPE correction: about 0.00891 eV;
* :math:`\Delta G_{\text{ads}}(298\,\text{K})`: about -0.16293 eV;
* :math:`\Delta G_{\text{ads}} = 0` crossover: about 448.887 K.
...
Before treating :math:`\Delta G` or ``T_cross`` as publication-level
...
and examine the stability of the frequencies, ZPE, :math:`\Delta F_{\text{vib}}`,
:math:`\Delta G`, and ``T_cross``.
...
With ``fail_on_imaginary = true``, a significant imaginary mode terminates the
thermochemical stage rather than being silently discarded. A low-frequency or
imaginary result should instead trigger inspection of the optimized geometry,
displacement amplitude, slab model, frozen-layer definition, and convergence.
...
using quantitative :math:`\Delta G` or ``T_cross`` values for publication,
test at least surface-cell size, slab thickness, number of frozen layers,
finite-difference displacement, and the treatment of low-frequency modes.
...
The calculated :math:`\Delta G_{\text{ads}}` and ``T_cross`` are pressure
...
:math:`\Delta F_{\text{vib}}`, :math:`\Delta U_{\text{vib}}`, and
:math:`\Delta S_{\text{vib}}`.
...
:math:`\Delta E_{\text{ads}} < 0`.

Nevertheless, adsorption removes the large translational entropy associated
with a gas-phase Hg atom. Thus :math:`\Delta S_{\text{ads}} < 0`, and the term
:math:`-T \Delta S_{\text{ads}}` becomes increasingly positive as temperature
rises.

The calculated :math:`\Delta G_{\text{ads}}` therefore becomes less favorable
with increasing temperature in the documented pressure range. The crossover
temperature is the point where the favorable electronic and vibrational
contributions are balanced by the gas-phase thermodynamic penalty.
...
10. Report temperature, Hg pressure, gas reference pressure, and ZPE
    convention together with :math:`\Delta G` and ``T_cross``.

The resulting :math:`\Delta G(T)` and ``T_cross`` should be regarded as
...
and therefore :math:`\Delta E_{\text{ads}} = -0.5013228844` eV.
...
cm\ :sup:`-1` and the Hg/Au frequencies span 4.281--140.090
cm\ :sup:`-1`. Two Hg-dominated Hg/Au modes are identified at approximately
19.312 and 19.517 cm\ :sup:`-1` using the reported projection threshold of
0.200.
...
:math:`T_{\text{cross}} = 235.025029` K, with the bisection residual reported
as :math:`\Delta G_{\text{ads}}(T_{\text{cross}}) = -3.16 \times 10^{-8}` eV.
...
:math:`\Delta G_{\text{ads}}` less favorable. Over this pressure interval, the
crossover temperature correspondingly decreases from about 451 K at 1 bar to
235 K at 1e-5 bar.

20.2 Standard-vibrational temperature scan at multiple pressures
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

For the standard vibrational treatment, :math:`\Delta H_{\text{ads}}` is
essentially pressure-independent because the pressure enters through the gas
entropy and chemical potential rather than through the electronic or solid
vibrational terms.
...
:math:`\Delta \text{ZPE} = 0.00637762` eV.
...
:math:`T_{\text{cross}} = 237.957615` K, with a reported residual of
approximately :math:`1.41 \times 10^{-8}` eV.
...
:math:`\Delta E_{\text{ads}} = -0.5013228844` eV.
...
20.1--20.4 are ASE-backend production results. They should be distinguished
from the project-custom equations/backend discussed elsewhere in this
document.
...
contain a separate ``[restart]`` section for reading previously saved relaxed
...
the computational model whose :math:`\Delta G` and ``T_cross`` are reported.
